...
elbow_stretching.py
===================
Real-time left-arm Elbow Stretching exercise tracker.

Workout structure: 3 sets × 10 reps with 30s cooldown between sets.
A rep counts only after holding the STRETCHED position for 2 seconds.

Usage
-----
    python elbow_stretching.py
    python elbow_stretching.py --camera 1
    python elbow_stretching.py --gif /path/to/demo.gif

Press 'q' or ESC to quit at any time.
Press 'e' to exit current set early.
"""
...
# Hold duration for a rep (seconds)
HOLD_DURATION = 2.0
...
class ExerciseStateMachine:
    ...
    def __init__(self) -> None:
        ...
        self.reps = 0
        self.peak_angle: float = 180.0      # lowest angle in current set (best curl)
        self.all_angles: list[float] = []
        self._hold_start: float | None = None
        self.hold_progress: float = 0.0

    def reset_for_new_set(self) -> None:
        """Reset state for a new set. Peak angle resets to 180."""
        self.state = self.RELAXED
        self.reps = 0
        self.peak_angle = 180.0
    ...
    def update(self, angle: float) -> str:
        ...
        if self.state == self.RELAXED:
            ...
            if angle < RELAXED_ANGLE:
                self.state = self.CURLING

        elif self.state == self.CURLING:
            self.hold_progress = 0.0
...
def draw_arm_overlay(frame, shoulder_px, elbow_px, wrist_px, color, angle):
    cv2.line(frame, shoulder_px, elbow_px, color, LINE_THICKNESS)
    cv2.line(frame, elbow_px, wrist_px, color, LINE_THICKNESS)
    cv2.circle(frame, shoulder_px, CIRCLE_RADIUS, color, cv2.FILLED)
    cv2.circle(frame, elbow_px, CIRCLE_RADIUS + 2, color, cv2.FILLED)
    cv2.circle(frame, wrist_px, CIRCLE_RADIUS, color, cv2.FILLED)

    text_pos = (elbow_px[0] + 15, elbow_px[1] - 15)
...
def draw_hud(frame, sm, angle, color, current_set, total_sets):
    ...
    # Title
    cv2.putText(frame, "ELBOW STRETCHING", (12, y), FONT, 0.7, COLOR_WHITE, 2, cv2.LINE_AA)
    y += 8
    ...
    peak_display = sm.peak_angle if sm.peak_angle < 180.0 else 0.0
    cv2.putText(frame, f"{peak_display:.1f}", (140, y), FONT, 0.7,
...
def draw_cooldown_screen(frame, remaining_secs: float, completed_set: int, total_sets: int):
    """Draw a dark cooldown screen with a countdown timer."""
    h, w = frame.shape[:2]
    ...
    # Title
    cv2.putText(frame, "COOLDOWN", (center_x - 120, h // 2 - 100),
                FONT, 1.5, COLOR_COOLDOWN, 3, cv2.LINE_AA)

    # Set completed info
    text = f"Set {completed_set} of {total_sets} complete!"
    cv2.putText(frame, text, (center_x - 170, h // 2 - 40),
                FONT, 0.8, COLOR_WHITE, 2, cv2.LINE_AA)

    # Countdown number
    secs_left = max(0, int(math.ceil(remaining_secs)))
    cv2.putText(frame, f"{secs_left}", (center_x - 30, h // 2 + 60),
                FONT, 3.0, COLOR_GREEN, 5, cv2.LINE_AA)
    cv2.putText(frame, "seconds", (center_x - 55, h // 2 + 100),
    ...
    bar_y = h // 2 + 130
    progress = 1.0 - (remaining_secs / COOLDOWN_SECS)
    progress = max(0.0, min(1.0, progress))

    cv2.rectangle(frame, (bar_x, bar_y), (bar_x + bar_w, bar_y + bar_h), COLOR_BAR_BG, cv2.FILLED)
    fill = int(bar_w * progress)
    if fill > 0:
        cv2.rectangle(frame, (bar_x, bar_y), (bar_x + fill, bar_y + bar_h), COLOR_COOLDOWN, cv2.FILLED)
    cv2.rectangle(frame, (bar_x, bar_y), (bar_x + bar_w, bar_y + bar_h), COLOR_WHITE, 1)

    # Next set hint
    if completed_set < total_sets:
        cv2.putText(frame, f"Next: Set {completed_set + 1}", (center_x - 80, bar_y + 60),
                    FONT, 0.7, (130, 130, 130), 1, cv2.LINE_AA)

    cv2.putText(frame, "Press 'q' or ESC to quit", (center_x - 130, h - 30),
                FONT, 0.5, (100, 100, 100), 1, cv2.LINE_AA)
...
def draw_summary_screen(frame, set_results: list[dict], was_early_exit: bool):
    """Draw the workout complete summary with per-set peak angles."""
    h, w = frame.shape[:2]
    frame[:] = (20, 20, 20)
    ...
    cv2.putText(frame, title, (center_x - 200, 60),
                FONT, 1.3, title_color, 3, cv2.LINE_AA)

    cv2.putText(frame, "ELBOW STRETCHING", (center_x - 120, 100),
                FONT, 0.7, COLOR_WHITE, 2, cv2.LINE_AA)

    # Table header
    y = 150
    cv2.line(frame, (center_x - 250, y), (center_x + 250, y), (80, 80, 80), 1)
    y += 30
    cv2.putText(frame, "SET", (center_x - 220, y), FONT, 0.6, (160, 160, 160), 1, cv2.LINE_AA)
    cv2.putText(frame, "REPS", (center_x - 80, y), FONT, 0.6, (160, 160, 160), 1, cv2.LINE_AA)
    cv2.putText(frame, "PEAK ANGLE", (center_x + 60, y), FONT, 0.6, (160, 160, 160), 1, cv2.LINE_AA)
    y += 10
    cv2.line(frame, (center_x - 250, y), (center_x + 250, y), (80, 80, 80), 1)
    y += 30
    ...
    for i, result in enumerate(set_results):
        ...
        cv2.putText(frame, f"Set {i + 1}", (center_x - 220, y), FONT, 0.7,
                    COLOR_WHITE, 2, cv2.LINE_AA)
        cv2.putText(frame, f"{reps}", (center_x - 60, y), FONT, 0.7,
                    set_color, 2, cv2.LINE_AA)
        peak_str = f"{peak:.1f}" if peak < 180.0 else "—"
        cv2.putText(frame, peak_str, (center_x + 80, y), FONT, 0.7,
                    (100, 255, 200), 2, cv2.LINE_AA)
        y += 40
    ...
    # Total reps
    cv2.putText(frame, "Total Reps:", (center_x - 220, y), FONT, 0.7,
                (160, 160, 160), 1, cv2.LINE_AA)
    cv2.putText(frame, f"{total_reps}", (center_x + 80, y), FONT, 0.8,
                COLOR_CYAN, 2, cv2.LINE_AA)
    y += 40

    # Average of peak angles
    if peak_angles:
        avg_peak = float(np.mean(peak_angles))
        cv2.putText(frame, "Avg Peak Angle:", (center_x - 220, y), FONT, 0.7,
                    (160, 160, 160), 1, cv2.LINE_AA)
        cv2.putText(frame, f"{avg_peak:.1f}", (center_x + 80, y), FONT, 0.8,
                    (100, 255, 200), 2, cv2.LINE_AA)

    # Exit hint
    cv2.putText(frame, "Press any key to exit", (center_x - 110, h - 30),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Elbow Stretching — real-time bicep curl tracker")
...
def main() -> None:
    ...
    print(f"[INFO] Hold stretched for {HOLD_DURATION}s to count a rep.")
    print("[INFO] Press 'e' to exit a set early, 'q'/ESC to quit.\n")
    ...
    current_set = 1
    set_results: list[dict] = []   # {"reps": int, "peak_angle": float}
    was_early_exit = False
    quit_requested = False

    # Application phases: "workout", "cooldown", "summary"
    phase = "workout"
    ...
    try:
        while True:
            ...
            if phase == "workout":
                ...
                if result.pose_landmarks and len(result.pose_landmarks) > 0:
                    ...
                    elbow = extract_landmark_xy(landmarks, LEFT_ELBOW)
                    wrist = extract_landmark_xy(landmarks, LEFT_WRIST)

                    raw_angle = compute_elbow_angle(shoulder, elbow, wrist)
                    ...
                    elbow_px = normalized_to_pixel(elbow, w, h)
                    wrist_px = normalized_to_pixel(wrist, w, h)

                    draw_arm_overlay(frame, shoulder_px, elbow_px, wrist_px, color, smooth_angle)
                ...
                if sm.reps >= REPS_PER_SET:
                    peak = sm.peak_angle if sm.peak_angle < 180.0 else 0.0
                    set_results.append({"reps": sm.reps, "peak_angle": sm.peak_angle})
                    print(f"[INFO] Set {current_set} complete! Reps: {sm.reps}, Peak: {peak:.1f}°")
            ...
            # Show
            cv2.imshow("Elbow Stretching Tracker", frame)
            ...
            # 'e' = exit current set early (only in workout phase)
            ...
            # In summary phase, any key (except q/e handled above) exits
        ...
        print("\n" + "=" * 55)
        print("  ELBOW STRETCHING — SESSION SUMMARY")
        print("=" * 55)
        total_reps = 0
        peak_angles = []
        ...
        if was_early_exit:
            print(f"  Note: Workout ended early by user.")
        print("=" * 55 + "\n")

        # Structured JSON output for API consumption
        import json
        result_json = {
            "exercise": "elbow_stretching",
